...
public class AdvancedStorageSettings extends SettingsPreferenceFragment {
    ...
    private boolean setSwitchablePair() {
        ...
        if (TextUtils.isEmpty(primaryStorage) || TextUtils.isEmpty(secondaryStorage)) {
            Log.e(TAG,"Unable to find two properly configured storage devices for vold switch");
            return false;
        }

        SystemProperties.set(VOLD_SWITCHABLEPAIR_PROP, primaryStorage + ',' + secondaryStorage);
        Log.i(TAG, "System property set: " + VOLD_SWITCHABLEPAIR_PROP + "=" + primaryStorage + ',' + secondaryStorage);
        return true;
    }
